...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if not self.config.trainer.sim2real:
                ...
                gen_config = GenerationConfig(
                    max_turns=self.config.data.max_turns,
                    max_start_length=self.config.data.max_start_length,
                    max_prompt_length=self.config.data.max_prompt_length,
                    max_response_length=self.config.data.max_infer_length,
                    max_obs_length=self.config.data.max_obs_length,
                    num_gpus=self.config.trainer.n_gpus_per_node,
                    no_think_rl=False,
                    topk = 3,
                    type=self.config.trainer.type
                )
                ...
                if not self.config.trainer.use_entropy:
                    test_output_gen_batch_padded, _ = generation_manager.run_llm_loop(test_gen_batch_padded, '', initial_input_ids=first_input_ids)
                else:
                    test_output_gen_batch_padded, _ = generation_manager.run_llm_loop_entropy(test_gen_batch_padded, '', initial_input_ids=first_input_ids)                    
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    # Create information loss mask to exclude information-bearing tokens from loss calculation
                    if self.config.actor_rollout_ref.actor.get("enable_information_loss_mask", False):
                        batch.batch["information_loss_mask"] = self.create_information_loss_mask(batch)
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            ...
                            reward_tensor, reward_tensor_format, reward_tensor_em, reward_extra_infos_dict, search_num = compute_reward(batch, self.reward_fn)
                    
                    debug_metric = {
                        'actor/tool_num': search_num,
                        'training/format_reward': reward_tensor_format
                    }

                    metrics.update(debug_metric)
